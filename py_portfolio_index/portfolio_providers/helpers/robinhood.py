    ...
    if "id" in data:
        ...
        while time.time() - start_time < 120:  # 2 minutes
            ...
            if challenge_response["challenge_status"] == "expired":
                raise ConfigurationError("Expired challenge.")
            if challenge_response["challenge_status"] == "validated":
                ...
                if inquiries_response["type_context"]["result"] == "workflow_status_approved":
                    ...
                    raise ConfigurationError("workflow status not approved")
            challenge_response = request_get(url=challenge_url)
            print("Waiting for challenge to be validated")
            print(time.time() - start_time)
    raise ConfigurationError("Id not returned in user-machine call")
...
def login(
    ...
    if os.path.isfile(pickle_path):
        ...
        if store_session:
            try:
                ...
                print(f"ERROR: There was an issue loading pickle file {e!s}. Authentication may be expired - logging in normally.")
    ...
    if data:
        if "challenge" in data:
            ...
            raise ConfigurationError(data.get("detail", str(data)))
        ...
        raise ConfigurationError("Error: Trouble connecting to robinhood API. Check internet connection.")
    return data
