...
from py_portfolio_index.portfolio_providers.local_dict import LocalDictProvider, LocalDictNoPartialProvider
from py_portfolio_index.operators import generate_composite_order_plan_v2, OrderElement, OrderType, OrderPlan
...
def test_composite():
    provider1 = LocalDictProvider(
        holdings=[
            RealPortfolioElement(ticker="AAPL", units=.5, value=Money(value=50)),
    ...
    composite_order_plan = generate_composite_order_plan_v2(composite, ideal_port, target_size=expected_size, purchase_order_maps=PurchaseStrategy.LARGEST_DIFF_FIRST)
    
    no_partial = composite_order_plan[LocalDictNoPartialProvider.PROVIDER]
    assert no_partial.to_buy == [
        OrderElement(ticker="MSFT", order_type= OrderType.BUY, value=None, qty=5, price=Money(value=33), provider=Provider.LOCAL_DICT_NO_PARTIAL),
    ...
    assert partial.to_buy == [
        OrderElement(ticker="AAPL", order_type= OrderType.BUY, value=Money(value='759.9999999999999644728632120'), qty=None, price=Money(value=100), provider=Provider.LOCAL_DICT),
    ]
